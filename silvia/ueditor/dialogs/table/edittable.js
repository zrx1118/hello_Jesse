...
(function () {
    var title = $G("J_title"),
        ...
        customSize = $G("J_customSize"),
        tone = $G("J_tone"),
        me,
        preview = $G("J_preview"),
        Jwidth = $G("J_lineWidth"),
        Jheight = $G("J_lineHeight");
    ...
    editTable.prototype = {
        init:function () {
            ...
            var cell_attr = me.getCellWidthAndHeight();
            Jwidth.value = cell_attr[0];
            Jheight.value = cell_attr[1];
            ...
            domUtils.on(customSize, "click", me.customSizeHandler);
        ...
        autoSizeContentHanler:function () {
            ...
            domUtils.setStyle($G("J_customDiv"), 'display', 'none');
        },
        ...
        customSizeHandler:function () {
            domUtils.setStyle($G("J_customDiv"), 'display', 'inline-block');
        },
         ...
         * @return {[*,*]}
         */
        getCellWidthAndHeight:function(){
            var start = editor.selection.getStart(),
                cell = domUtils.findParentByTagName(start, ["td", "th", "caption"], true);
            var width = cell && cell.getAttribute("width");
            var height = cell && cell.getAttribute("height");
            if(!width){ width = '32'};
            if(!height){ height = '16'};
            return [width, height];
        },
    ...
    dialog.onok = function () {
        ...
        if(customSize.checked){
            var width = Jwidth.value;
            var height = Jheight.value;
            if(!height){
                alert("高度设置不能为空！");
                Jheight.focus();
                return false;
            ...
            editor.execCommand("adaptbycustomer", width, height);
        }
        editor.fireEvent('saveScene');
